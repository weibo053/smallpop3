...
void handle_client(int s)
{
     ...
     */
    while( (inputline = readline(cl)) && pop3_fsm(inputline) ) continue;
    fclose(cl);
}
...
int pop3pass(void){
    ...
    if(!mbox) FATALerror("Cannot open mailbox\n");
    scanmbox(mbox);
    fprintf(cl,"+OK\n");
    return TRANSACTION;
}
...
int pop3_fsm(char *input)
{
    ...
    if(cmd){
        ...
        case AUTHORIZATION:
            if(strcasecmp(cmd,"USER")==0) return serverstate = pop3user();
            if(strcasecmp(cmd,"PASS")==0) return serverstate = pop3pass();
            if(strcasecmp(cmd,"QUIT")==0) return serverstate = pop3quit();
            break;
        case TRANSACTION:
            if(strcasecmp(cmd,"STAT")==0) return serverstate = pop3stat();
            if(strcasecmp(cmd,"LIST")==0) return serverstate = pop3list();
            if(strcasecmp(cmd,"RETR")==0) return serverstate = pop3retr();
            if(strcasecmp(cmd,"TOP")==0) return serverstate = pop3top();
            if(strcasecmp(cmd,"QUIT")==0) return serverstate = pop3quit();
            break;
        }
    }
...
char *validate_user(char *name, char *pass)
{
    ...
    FILE *p;
    p = fopen("maillist","r");
    if(!p) FATALerror("Cannot open maillist\n");
    while(fscanf(p,"%s %s %s",uname,passwd,mailbox)!=EOF){
        if((strcmp(uname,name)==0)&&(strcmp(passwd,pass)==0)) return mailbox;
    }
    return NULL;
}
